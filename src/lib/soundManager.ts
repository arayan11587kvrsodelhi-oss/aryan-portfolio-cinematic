...
class SoundManager {
  ...
  public getAmplitude() {
    if (!this.analyser || !this.isEnabled) return 0
    const data = new Uint8Array(this.analyser.frequencyBinCount)
    this.analyser.getByteTimeDomainData(data)
    let sum = 0
    for (const sample of data) { const centered = (sample - 128) / 128; sum += centered * centered }
  ...
  public setVolume(vol: number) {
    ...
    if (this.masterGain && this.ctx) {
      this.masterGain.gain.setTargetAtTime(this.volume * 0.9, this.ctx.currentTime, 0.05)
    }
  }
  ...
  private playClick(now: number, level: number) {
    ...
    gain.gain.setValueAtTime(0.065 * level, now)
  ...
  private playProjectHover(now: number, level: number) {
    ...
    gain.gain.setValueAtTime(0.04 * level, now)
  ...
  private playProjectOpen(now: number, level: number) {
    ...
    gain.gain.linearRampToValueAtTime(0.065 * level, now + 0.08)
  ...
  private playModalOpen(now: number, level: number) {
    ...
    freqs.forEach((freq, idx) => {
      ...
      gain.gain.linearRampToValueAtTime((0.035 * level) / (idx + 1), now + 0.04)
  ...
  private playSuccess(now: number, level: number) {
    ...
    notes.forEach((n) => {
      ...
      gain.gain.setValueAtTime(0.065 * level, n.time)
